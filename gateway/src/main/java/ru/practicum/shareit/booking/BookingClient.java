...
public class BookingClient extends BaseClient {
  ...
  public ResponseEntity<Object> bookItem(long userId, BookItemRequestDto requestDto)
    ...
    if (requestDto.getEnd().isBefore(requestDto.getStart()) || requestDto.getEnd()
        .isEqual(requestDto.getStart())) {
      throw new ValidationException("Дата окончания должна быть позже даты старта");
    }

    if (isNull(requestDto.getStart()) || isNull(requestDto.getEnd()) || requestDto.getEnd()
        .isBefore(requestDto.getStart()) || requestDto.getEnd().isEqual(requestDto.getStart())) {
  ...
  public ResponseEntity<Object> getOwnerBookings(long userId, BookingState state, Integer from,
      Integer size) {
    Map<String, Object> parameters = Map.of("state", state.name(), "from", from, "size", size);
